...
if __name__ == '__main__':
    ...
    def get_classifier(num_points):
        classifier = PointNetCls(k=len(test_dataset.classes), num_points=num_points)
        if args.use_cuda:
            classifier.cuda()
    ...
    num_drops = 2048  # args.n_drops
    grad_cam = Saliency(get_classifier=get_classifier, num_classes=16, num_drop_points=num_drops,
                              heatmap_updates=False, cuda=args.use_cuda)
    ...
    drop_experiment = PointDropExperiment(
        ...
        high_drop=True,
        low_drop=True,
        file_prefix="exp5-pn1-saliency-",
        plot_title_prefix_text="EXP5 PcSM PointNet",

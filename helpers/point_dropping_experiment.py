...
class PointDropExperiment:
    ...
    def run_experiment(self):
        ...
        for batch_id, data in tqdm(enumerate(loader, 0), total=len(loader), smoothing=0.9):
            ...
            if self.print_progress:
                ...
                print(batch_id, 'done in', dur, 'seconds')
                print('estimated time untill complete:', (num_loader_iterations - batch_id - 1) * dur, 'seconds')
    ...
    def drop_experiment(
        ...
        cam = self.grad_cam(input, target)
        ...
        if self.insertion_experiment:
            ...
            initial_class_id = initial_result.data.argmax(1)
        else:
        ...
        # initial_loss = self.get_loss(initial_result, target)
        # self.random_loss[0] += initial_loss
        # self.high_loss[0] += initial_loss
        # self.low_loss[0] += initial_loss

        # add 0 drop accuracy values
        #initial_correct = np.equal(initial_class_id, target_idx).sum()
        initial_correct = initial_class_id.eq(target).sum().item()
        ...
        if self.high_drop:
            ...
            for i in range(self.num_drops):
                ...
                if self.heatmap_update_condition(i + 1):
                    # update cam
                    hcam = self.grad_cam(points, target)
                    if np.isnan(hcam).any():
                        hcam = np.zeros(hcam.shape)
                ...
                if self.measure_condition(i + 1):
                    ...
                    self.high_correct[k] += class_id.eq(target).sum().item()
                    k = k + 1
        ...
        if self.low_drop:
            ...
            lcam_add_gate = np.zeros(lcam.shape)
            j = 1

            for i in range(self.num_drops):
                idx = np.argmin(lcam, axis=1)
                # set cam value to one so that the next argmin call the return the next higher value

                for idx_j in range(len(idx)):
                    lcam[idx_j, idx[idx_j]] = 1.5
                    lcam_add_gate[idx_j, idx[idx_j]] = 1.5
                    points[idx_j, :, idx[idx_j]] = 0.0  # shift point to center
                result = None

                if self.heatmap_update_condition(i + 1):
                    # update cam
                    lcam = self.grad_cam(points, target)
                    if np.isnan(lcam).any():
                        lcam = np.zeros(lcam.shape)
                ...
                if self.measure_condition(i + 1):
                    ...
                    self.low_correct[j] += class_id.eq(target).sum().item()
                    j += 1
        ...
        self.num_classified += int(input.shape[0])
    ...
    def plot_results(self):
        ...
        for m in measures:
            ...
            if self.plot_title:
                plt.title(f"{self.plot_title_prefix} {self.num_classified} pcds, {self.num_drops} drops, {self.steps} step, {'hm updated' if self.update_cam else 'hm not updated'}")
            if m[0] == 'Loss':
                plt.ylim(0.0)
            else:
    ...
    #     return torch.nn.functional.softmax(output).cpu()[:, list(target_idx)]

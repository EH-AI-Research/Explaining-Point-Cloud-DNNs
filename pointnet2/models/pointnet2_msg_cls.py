...
class Pointnet2MSG(nn.Module):
    ...
    def forward(self, pointcloud):
        ...
        for module in self.features:
            xyz, features, sample_inds = module(xyz, features)

            if hasattr(module, 'save_features_progress'):
                ...
                module.save_features_progress(features, xyz, sample_inds)

            if hasattr(module, 'save_features_progress_input'):
                # this function only has been added to the last set abstraction module
                # in the vote feature map before max and classification
                module.save_features_progress_input(features, xyz, sample_inds)

            if hasattr(module, 'save_features_progress_input_mp'):
                # this function only has been added to the last set abstraction module
                # in the vote feature map before max and classification
                module.save_features_progress_input_mp(features, xyz, sample_inds)

            if hasattr(module, 'save_features_interp'):  # TODO
                module.save_features_interp(features, xyz, sample_inds)
